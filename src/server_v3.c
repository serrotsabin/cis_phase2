...
int num_clients = 0;
int current_controller = -1; // Index of current controller (-1 = none)

// Control request queue
request_t request_queue[MAX_CLIENTS];
int queue_head = 0; // Added
int queue_tail = 0; // Added
    ...
    while (1)
    {
        ...
        for (int i = 2; i < nfds; i++)
        {
            if (fds[i].revents & POLLIN)
            {
                ...
                if (n > 0)
                {
                    ...
                    for (int j = 0; j < n; j++)
                    {
                        ...
                        else if (ch == 24)
                        {
                            ...
                            should_remove = 1;
                            ctrl_handled = 1;
                            break; // Exit the byte loop
                        }
                    }

                    // Handle disconnect after processing all bytes
                    if (should_remove)
                    {
                ...
                {
                    // Read returned 0 or -1 → client disconnected
                    remove_client(client_idx);
                }
            }
